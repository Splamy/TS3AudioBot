...
## 3.3 Note
From this point on the crypto process is completed. The normal encryption
scheme (see 1.6) is from now on used. All `Command`, `CommandLow`, `Ack`,
`AckLow` packets must get encrypted.
`Voice` (and `VoiceWhisper` when needed) packets should be encrypted when the
channel encryption or server wide encryption flag is set.
`Ping` and `Pong` must not be encrypted.

## 3.4 clientinit (Client -> Server)
...
### 3.6.1 channellist and channellistfinished
...
### 3.6.2 notifycliententerview
Same as the query notification.

### 3.6.3 notifychannelgrouplist
### 3.6.4 notifyservergrouplist
### 3.6.5 notifyclientneededpermissions
...
can only calculate the uid of your own identity and the server identity when
connecting.
