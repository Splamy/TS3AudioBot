...
namespace TS3Client.Full
{
	...
	internal struct Packet<TDir>
	{
		...
		{
			Raw = new byte[data.Length + HeaderLength + Ts3Crypt.MacLen];
		...
		public void FromHeader()
		{
			...
			if (typeof(TDir) == typeof(S2C))
			{
				PacketId = BinaryPrimitives.ReadUInt16BigEndian(rawSpan.Slice(Ts3Crypt.MacLen));
				PacketTypeFlagged = Raw[Ts3Crypt.MacLen + 2];
			}
			else if (typeof(TDir) == typeof(C2S))
			{
				var ext = new C2S();
				PacketId = BinaryPrimitives.ReadUInt16BigEndian(rawSpan.Slice(Ts3Crypt.MacLen));
				ext.ClientId = BinaryPrimitives.ReadUInt16BigEndian(rawSpan.Slice(Ts3Crypt.MacLen + 2));
				PacketTypeFlagged = Raw[Ts3Crypt.MacLen + 4];
